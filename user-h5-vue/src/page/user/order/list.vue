  ...
  <div>
    ...
    <div v-for="(item,index) in list" :key="index">
      <van-cell-group class="order-item">
        <van-panel :title="'订单：'+item.ordercode" :orderStatus="item.state">
          <div slot="header">
...
<!--              </div>-->
              <div v-if="item.products.length > 0" class="more">
                <div class="item" v-for="(product,i) in item.products" :key="i">
                  <div class="products-item">
                    <img :src="product.skuImage"/>
                    <span class="product-title">{{product.skuName}}</span>
                    <span class="product-title">￥{{product.buyPrice / 100}} x{{product.quantity}}</span>
                  </div>
                </div>
              </div>
            </router-link>
          ...
          <div slot="footer" class="footer">
            ...
            <router-link :to="'/user/order/logistics/'+item.orderid">
              <van-button v-if="[3,4,5].indexOf(item.orderStatus) != -1" size="small">查看物流</van-button>
            </router-link>
            <van-button v-if="item.orderStatus === 3 " size="small" v-on:click="clickConfirmReceiving(item)">确认收货</van-button>
            <van-button v-if="item.orderStatus === 10 " size="small" type="danger" @click="goPay(item.orderid)">
                    支付
            </van-button>

            <router-link :to="'/user/aftersale/apply/'+item.orderid">
              <van-button v-if="item.orderStatus !== 10 && item.orderStatus !== 5" size="small" type="danger">
                申请售后
              </van-button>
            </router-link>
          </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      onTabChange(index) {
        // orderStatus 和 tab index 对应的关系
        const statusArray = [null, 10, 20, 30];
        // if (index === 0) {
        //   // 全部
        // } else if (index === 1) {
        //   // 待付款 1
        ...
        getOrderPage({
          ...
          const list = orders.map(order => {
            ...
            return {
              ...
              state: statusArray[order.orderStatus],
